...
- 아직 push하지 않았으므로 이 변경 SHA의 GitHub CI는 실행하지 않았다. push와 원격 CI 확인은 사용자의 별도 승인을 받은 뒤에만 한다.
- 제3자 의존성 전체의 라이선스 호환성·고지 의무를 감사한 것은 아니다. AIOS의 MIT 선언이 제3자 라이선스를 바꾸지 않는다.
- 공개 정체성 `JongHyun (jonghyun0000)`을 저작권자로 사용했다. 사용자가 법적 실명이나 다른 권리 주체를 원하면 **push 전에** 바꿔야 한다.

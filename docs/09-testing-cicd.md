...
- **의존성 감사 게이트(현행 `ci.yml`):** 별도 `audit` job 이 `pnpm run audit:prod`(`--prod --audit-level=high`)를 실행하고, 주 1회 스케줄로도 돈다. 위의 배포 파이프라인 도식은 설계안이고 현재 활성 CI 는 정적 검사·빌드·단위·데모/접근성 회귀에 이 감사를 더한 것이다. 근거와 범위는 [보안 §15.5](08-security.md).

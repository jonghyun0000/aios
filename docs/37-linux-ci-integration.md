...
위 승인 대기 상태 이후 사용자가 `028e2e2`의 push를 승인했다. [실행 36023837723](https://github.com/jonghyun0000/aios/actions/runs/36023837723)의 integration 잡은 UTC 2026-09-24 15:56:36~15:58:33, **117초(1분 57초)**에 성공했다. 서비스 이미지·모델 최초 다운로드와 종료 정리까지 포함해 사전 상한 20분을 충족한다. 로그는 `/Volumes/T7/bigdata/verification-reports/ci-integration-remote-36023837723.log`에 보존했다.

- Ollama **0.32.15**, Linux CPU에서 실제 실행. 모델 ID는 qwen2.5:0.5b `a8b0c5157701`, bge-m3 `790764642607`.
- 마이그레이션 0005 누락을 같은 검사기의 `42P01` 실패로 검출한 다음 정상 마이그레이션 경로를 실행했다.
- 새 대화 **3/3**: SSE·PostgreSQL·Redis 내용 일치, Wilson 95% **43.85%~100%**. 모델 품질이나 일반 업무 성공률은 뜻하지 않는다.
- 실제 임베딩 **1024차원**·pgvector 왕복, 승인 전 파일 불변·HTTP 승인·독립 SHA-256 확인·중복 승인 거부·원본 복구가 모두 PASS.
- 마이그레이션 원본 지문은 로컬과 동일하다. 검사 프로세스와 서비스 컨테이너 정리도 정상 종료했다.

단계 2-1의 완료 근거는 **`028e2e2`와 이 원격 실행**이다. 첫 실패는 위에 유지한다. macOS 클린룸·다른 Mac·모델 도구 선택 품질·별도 API 프로세스 자연 종료·사용자 백업 복원은 여전히 미검증이며 점수는 변경하지 않는다.

동일 SHA의 전체 워크플로도 **success**로 종료했다: `audit`, `verify`, `integration` **3/3** 성공. 이 후속 커밋은 실행 근거·상태 문서만 갱신하며 제품·검사 코드는 수정하지 않는다.

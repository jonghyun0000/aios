...
- **신규 기능:** 공공통계 상세의 선택 필터·수치 요약을 새 채팅의 편집 가능한 AI 분석 초안으로 넘긴다. 자동 전송·원본 변경 없이 탭에서 한 번만 전달하며, `343a00c`의 원격 CI 3/3 성공과 실행 근거는 `docs/38`이다.
- **다음 작업:** **2-1 완료(`028e2e2`)**. 원격 audit·verify·integration 모두 성공, 실제 서비스 통합은 다운로드 포함117초(`docs/37`). 공개 오픈소스 방향의 다음 항목은 NEXT_STEPS **3-3 신규 사용자 관찰**이며 섭외는 사용자 결정이다. 긴 이력은 [CHANGELOG.md](CHANGELOG.md)에 보존했다.
...
- **CI 범위를 구분한다.** 공개 저장소의 `.github/workflows/ci.yml`은 Node 22 정적 검사·빌드·머신 독립 단위 검사, 정적 데모와 실제 앱의 합성 API/WS 접근성·키보드 회귀를 수행한다.
  `integration` 잡은 실제 서비스·마이그레이션·대화 3회·파일 승인·복구 경로를 검사한다. 최초 HTTP 400 실패 이후 버전을 정렬한 `028e2e2`에서 원격 성공했다(117초). 로컬·원격 근거와 한계는 `docs/37`을 따른다.

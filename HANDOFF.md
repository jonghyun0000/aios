...
- **다음 작업:** [NEXT_STEPS.md](NEXT_STEPS.md)의 **1-3 API 오류 분류 core UUID 감사 확장**. 긴 과거 이력은 [CHANGELOG.md](CHANGELOG.md)에 원문 보존했다. 과거 성공을 새 변경의 검증으로 세지 않는다.
...
- **1-3 진행(2026-09-22):** `3a02beb`에서 메시지 UUID와 실제 TCP 이탈 분류를 구현·검증했다. 전체 라우트 감사에서 core UUID 경로 3개를 추가 발견해 `docs/33` §3.1 기준으로 확장 중이다.
...
| 32 | API 오류 분류 잡음 — **진행(2026-09-22)** | `3a02beb`에서 메시지 GET·POST UUID와 실제 소켓 이탈을 수정·결함 주입했다. 후속 전체 라우트 감사에서 project index/search·memory delete도 잘못된 UUID를 500으로 반환함을 실제 로컬 PostgreSQL에서 확인해 같은 1-3 범위로 확장 중이다. 상세 `docs/33-api-error-classification.md` |
